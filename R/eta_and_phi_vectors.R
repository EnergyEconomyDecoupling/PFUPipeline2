...
sum_phi_vecs <- function(phi_pf_vecs,
  ...
  if (nrow(temp) == 0) {
    ...
    if (!all(err_check[[.phi_sum_OK]])) {
      ...
      err_msg <- paste("In PFUPipeline2::sum_phi_vecs(), the length of the sum of phi_pf and phi_u vectors",
    ...
    if (!all(err_check[[.phi_cols_OK]])) {
      ...
      err_msg <- paste("In PFUPipeline2::sum_phi_vecs(), the names of the phi.pf and phi.u columns should be the same.",
